  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-28 md:pb-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* Week Selector & Action button */}
        <div className="flex items-center justify-between md:justify-end gap-3 pt-2 md:pt-0 border-t md:border-t-0 border-slate-100 flex-wrap">
          {onSelectWeek && (
          ...
          <button
            ...
            className={`px-4 sm:px-5 py-2 sm:py-2.5 font-bold rounded-xl text-xs sm:text-sm shadow-md transition-all flex items-center space-x-2 ${
              isRegistrationOpen
                ? 'bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer active:scale-95'
                : 'bg-slate-200 text-slate-400 cursor-not-allowed'
            }`}
          >
            {isRegistrationOpen ? <Check className="w-4 h-4" /> : <Lock className="w-4 h-4" />}
            <span>{isRegistrationOpen ? 'Lưu Đăng Ký' : 'Đã Khóa Đăng Ký'}</span>
          </button>
        </div>
      </div>
        ...
        <div className="bg-amber-50 border border-amber-300 rounded-2xl p-4 sm:p-5 text-amber-900 flex items-start space-x-3.5 shadow-2xs">
          ...
          <div className="space-y-1">
            <div className="font-bold text-sm sm:text-base flex items-center space-x-1.5 text-amber-950">
              <span>Cổng Đăng Ký Ca Tuần Này Đang Đóng</span>
            </div>
            <p className="text-xs sm:text-sm text-amber-800 leading-relaxed">
              Quản lý chưa mở hoặc đã đóng cổng nhận đăng ký ca cho <strong>{solarRange}</strong>. Bạn đang ở chế độ xem lại nguyện vọng đã lưu. Nếu cần đăng ký thêm hoặc đổi ca, vui lòng liên hệ Quản lý chi nhánh để mở cổng.
            </p>
          </div>
        </div>
      ) : (
        <div className="bg-emerald-900 text-white rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-3 sm:gap-4">
          <div className="flex items-start space-x-2.5 sm:space-x-3">
            ...
            <div className="text-xs text-emerald-100 space-y-1">
              <p className="font-bold text-white text-sm">
                Cổng Đăng Ký Đang Mở — Được đăng ký cả 3 ca / ngày:
              </p>
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2 shrink-0 w-full sm:w-auto">
            <button
              ...
              className="px-2.5 py-1.5 bg-emerald-800 hover:bg-emerald-700 border border-emerald-700 text-emerald-100 text-[11px] sm:text-xs font-semibold rounded-lg transition-colors cursor-pointer text-center"
            >
              + Ca Sáng
            </button>
            <button
            ...
            <button
              ...
              className="px-2.5 py-1.5 bg-emerald-700 hover:bg-emerald-600 border border-emerald-600 text-white text-[11px] sm:text-xs font-bold rounded-lg transition-colors cursor-pointer text-center flex items-center space-x-1"
            >
            ...
            <button
              ...
              className="px-2.5 py-1.5 bg-emerald-800 hover:bg-emerald-700 border border-emerald-700 text-rose-300 text-[11px] sm:text-xs font-semibold rounded-lg transition-colors cursor-pointer flex items-center space-x-1"
            >
      ...
      <div className="md:hidden space-y-3">
        <div className="text-xs font-bold text-slate-700 uppercase tracking-wide flex items-center justify-between">
          <span>Chọn ngày theo Dương Lịch:</span>
          {!isRegistrationOpen && (
            <span className="text-[11px] text-amber-700 font-bold flex items-center">
              <Lock className="w-3 h-3 mr-0.5" /> Chỉ xem
            </span>
          )}
        </div>
          ...
          {DAYS_OF_WEEK.map((d) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-[9px] font-mono mt-0.5 px-1 rounded font-bold ${
                    countOnDay === 3
                      ? isSelectedDay
          ...
          <div className="flex items-center justify-between pb-2 border-b border-slate-100">
            <div>
              ...
              <span className="text-xs text-slate-500 ml-2">
                (Đang chọn {getDayCount(activeMobileDay)}/3 ca)
              </span>
            </div>
            {getDayCount(activeMobileDay) === 3 && (
              <span className="text-[10px] bg-amber-100 text-amber-900 font-bold px-2 py-0.5 rounded-full border border-amber-300 flex items-center space-x-1">
                <Sparkles className="w-3 h-3 text-amber-600" />
                <span>Đăng ký trọn vẹn 3 ca</span>
              </span>
            )}
          </div>
                ...
                {(['morning', 'afternoon', 'evening'] as ShiftType[]).map((shiftType) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-2.5 rounded-xl border transition-all flex flex-col justify-between ${
                        !isRegistrationOpen
                          ? active
                          ...
                          : active
                          ? 'bg-emerald-50 border-emerald-500 text-emerald-950 font-bold shadow-2xs ring-1 ring-emerald-400 cursor-pointer'
                          : 'bg-slate-50/70 border-slate-200 text-slate-600 hover:bg-slate-100/70 cursor-pointer'
                      }`}
                    >
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <div className="text-xs text-slate-500 font-medium">
              Sau khi lưu nguyện vọng ca, Quản lý chi nhánh {currentBranch.shortName} sẽ tổng hợp và xếp lịch làm việc chính thức.
            </div>
      ...
      <div className="md:hidden fixed bottom-14 left-0 right-0 z-30 bg-white/95 backdrop-blur-md border-t border-slate-200 px-4 py-2.5 shadow-lg flex items-center justify-between gap-3">
        <div>
          <div className="text-[11px] text-slate-500">
            Đã chọn: <span className="font-bold text-emerald-700 font-mono text-xs">{selectedCount} ca</span>
          </div>
        ...
        <button
          onClick={handleSave}
          disabled={!isRegistrationOpen}
          className={`px-5 py-2.5 font-bold rounded-xl text-xs shadow-md transition-all flex items-center space-x-1.5 ${
            isRegistrationOpen
              ? 'bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer active:scale-95'
              : 'bg-slate-200 text-slate-400 cursor-not-allowed'
          }`}
        >
          {isRegistrationOpen ? <Check className="w-4 h-4" /> : <Lock className="w-4 h-4" />}
          <span>{isRegistrationOpen ? 'Lưu Đăng Ký' : 'Đang Khóa'}</span>
        </button>
      </div>
    </div>
  );
